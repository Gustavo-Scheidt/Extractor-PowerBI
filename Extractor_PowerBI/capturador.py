...
def capturar_payloads_da_url(url_dashboard, pasta_destino="payloads", max_paginas=25):
...
def navegar_paginas_do_relatorio(page, estado, max_paginas=25):
    ...
    for i in range(1, max_paginas + 1):
        ...
        try:
            ...
            if btn_avancar.count() > 0 and btn_avancar.is_visible() and btn_avancar.is_enabled():
                _log_captura(f"Avançando para a página {i + 1}...")
                btn_avancar.click(force=True, timeout=5000)
                page.wait_for_timeout(3000)
            else:
                _log_captura("Chegou na última página. Fim da navegação.")
                break
        except Exception as e:
