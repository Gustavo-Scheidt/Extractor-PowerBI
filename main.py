...
def parse_args():
    ...
    p.add_argument(
        "--debug",
        action="store_true",
        help="Roda a captura com o navegador visível (não-headless) e mais devagar, para diagnosticar travamentos.",
    )
    return p.parse_args()
...
def main():
    ...
    capturar_payloads_da_url(url_dashboard, max_paginas=args.max_paginas, debug=args.debug)
    ...
    for nome, df in dataframes.items():
        print(f"{nome}: {df.shape[0]} linhas x {df.shape[1]} colunas")
        if not df.attrs.get("colunas_identificadas", False):
            nao_identificados.append(nome)
    print(f"Total de DataFrames: {len(dataframes)}")

    if nao_identificados:
        print(f"\n⚠️  {len(nao_identificados)} visual(is) com colunas NÃO identificadas (nomes genéricos 0,1,2...):")
        for nome in nao_identificados:
            print(f"   - {nome}")
        print("   Revise essas abas/CSVs manualmente para nomear as colunas corretamente.")
        logger.warning(
            "%d visual(is) com colunas não identificadas: %s",
            len(nao_identificados), ", ".join(nao_identificados),
        )
